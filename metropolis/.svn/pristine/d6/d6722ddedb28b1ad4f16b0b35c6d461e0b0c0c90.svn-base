var DataGrid = function(opts) {
	var defaults = {
		...
		multiboxonly : true,
		rowNum : 15, // 默认没页显示数
		rowList : [ 10, 15, 20, 30, 50 ], // 可选择记录条数
	...
	function fnLoadComplete() {
		// 整个底部移到顶上
		var gridPager = $(opts.pager);
		// 找到分页条
		var pagingPager = $(gridPager).find(".ui-paging-pager");

		pagingPager.find("#first_gridPager").html("首页");
		pagingPager.find("#prev_gridPager").html("上一页");
		pagingPager.find("#next_gridPager").html("下一页");
		pagingPager.find("#last_gridPager").html("尾页");

		// 去除输入框
		var input_html = pagingPager.find("#input_gridPager").html();
		pagingPager.find("#input_gridPager").remove();
		pagingPager.find(".ui-separator").remove();
		pagingPager.find("#last_gridPager").html("尾页");
		$(".ui-paging-pager").find("#last_gridPager").after(input_html);

		$("#gridPager_center").css("width", "inherit");
		$(".ui-pg-input").css("height", "20px").css("margin-top", "2px");
		$(".ui-pg-selbox").css("height", "22px");
		dataIsNullPoint();
	}
	 ...
	 * 参数: id需要选中行的ID 返回: 无
	 */
	this.setLabel = function(olname, nData, prop, attrp) {
		$(opts.pageid).jqGrid('setLabel', olname, nData, prop, attrp);
	}
