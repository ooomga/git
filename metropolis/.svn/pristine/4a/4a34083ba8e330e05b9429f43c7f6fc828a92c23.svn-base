	...
	<div>
		<form  id="addLineForm">
			...
			<span class="input-group-addon">所属线路</span> 
			 <c:if test="${empty dataStation.list[0].extraLineCode }">
			 	<input type="text" id="lineCode" name="lineCode" class="form-control"  value="${dataStation.list[0].lineCode}">
			 </c:if>
			 <c:if test="${not empty dataStation.list[0].extraLineCode }">
			 	<input type="text" id="lineCode" name="lineCode" class="form-control"  value="${dataStation.list[0].lineCode},${dataStation.list[0].extraLineCode}">
			 </c:if>
			</div>
