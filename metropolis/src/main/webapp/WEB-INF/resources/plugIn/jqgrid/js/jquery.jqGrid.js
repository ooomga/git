...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		updatepager = function(rn, dnd) {
			...
			if(pgboxes) {
				...
				if(ts.p.pginput===true) {
					$("#input"+tspg).html($.jgrid.template($.jgrid.getRegional(ts, "defaults.pgtext", ts.p.pgtext) || "","<input "+getstyle(stylemodule, 'pgInput', false, 'ui-pg-input') + " type='text' size='2' maxlength='7' value='0' role='textbox'/>","<label id='sp_1_"+$.jgrid.jqID(pgid)+"'></label>"));
					if(ts.p.toppager) {
						$("#input_t"+tspg_t).html($.jgrid.template($.jgrid.getRegional(ts, "defaults.pgtext", ts.p.pgtext) || "","<input "+getstyle(stylemodule, 'pgInput', false, 'ui-pg-input') + " type='text' size='2' maxlength='7' value='0' role='textbox'/>","<label id='sp_1_"+$.jgrid.jqID(pgid)+"_toppager'></label>"));
					}
		...
		setPager = function (pgid, tp){
			...
			if(ts.p.pginput===true) {
				$("#"+pgcnt).on('click','.js-change', function(e) {
					if (!clearVals('user', this)) {
						return false;
					}
					$('input.ui-pg-input').val( intNum( $('input.ui-pg-input').val(), 1));
					ts.p.page = ($('input.ui-pg-input').val()>0) ? $('input.ui-pg-input').val():ts.p.page;
					// 当输入超过最后一页时显示最后一页数据
					if (ts.p.lastpage <= ts.p.page) {
